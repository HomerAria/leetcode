...
class Solution {
    ...
    private int bruteForce(int[] gas, int[] cost){
        // choose a different start in each loop
        int number = gas.length;
        for(int start = 0; start < number; ++start){
            // set the start condition
            int rest = gas[start] - cost[start];
            int current = (start + 1) % number;

            // go around with a full circle
            while(rest >= 0 && current != start){
                rest = rest + (gas[current] - cost[current]);
                current = (current + 1) % number;
            }
            // end condition
            if(rest >= 0 && current == start){
                return start;
            }
        }
        return -1;
